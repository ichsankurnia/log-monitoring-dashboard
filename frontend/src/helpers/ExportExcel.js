...
class ExportExcel {
    static exportListTroubleET = async (user, srcDataTable) => {
        try {
            const columns = [
            ...
            workbook.creator = user.nama_user || 'Petugas Call Center';
            workbook.lastModifiedBy = user.nama_user || 'Petugas Call Center';
            ...
            signCellValue.value = `Export by:\n${user.nama_user || 'Petugas Call Center'}`
    ...
    static exportDocumentation = async (user, srcDataTable) => {
        try {
            const columns = [
                { header: 'No', key: 'number', width: 5, style: {numFmt: '0;[Red]0'}},
                { header: 'No Ticket', key: 'no', width: 25 },
    ...
    static exportFormAduan = async (user, data) => {
        try {
            ...
            worksheet.getCell('D8').value = user.nama_user || 'Petugas Call Center'
